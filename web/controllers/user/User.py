...
def edit():
    if request.method == "GET":
        return ops_render("user/edit.html", {"current": "edit"})  # current 前端按钮高亮 ,g.current_user = user_info
...
def reset_pwd():
    if request.method == "GET":
        return ops_render("user/reset_pwd.html", {"current": "reset-pwd"})  # current 前端按钮高亮
