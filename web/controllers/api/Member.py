...
def login():
    ...
    member_info = Member.query.filter_by(id=bind_info.member_id).first()  #拿到用户信息，生成token

    token = "%s#%s" % (MemberService.gene_auth_code(member_info), member_info.id)  # token信息
